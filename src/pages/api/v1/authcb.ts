...
const handler = async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
    const url = `https://www.strava.com/oauth/token?client_id=${process.env.NEXT_PUBLIC_STRAVA_CLIENT_ID}&code=${req.query.code}&grant_type=authorization_code&client_secret=${process.env.STRAVA_CLIENT_SECRET}`
